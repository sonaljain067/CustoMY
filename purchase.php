...
<body>
	...
	<div class="col-md-6 well">
		...
		<?php
			...
			$query = mysqli_query($conn, "SELECT * FROM `products` WHERE `id` = '$_REQUEST[id]'") or die(mysqli_error());
			$fetch = mysqli_fetch_array($query);
		?>
		<div class="col-md-2"></div>
		<div class="col-md-8">
			<img src="<?php echo $fetch['photo']?>" width="100%" height="300px"/>
			<h3><?php echo $fetch['name']?></h3>
			<h4>&#8369;<?php echo number_format($fetch['price'])?></h4>
			...
			<div class="form-group">
				...
				<input class="form-control" type="number" value="<?php echo $fetch['price']?>" id="total" readonly="readonly" lang="en-150"/>
			</div>
		</div>
	</div>
